'use client';

import { useEffect, useState } from 'react';
import { useRouter } from 'next/navigation';
import { useAuth } from '@/context/AuthContext';
import { LogIn } from 'lucide-react';
import { Toast } from '@/components/Toast';

export default function LoginPage() {
    const { user, loading, isAdmin, signInWithGoogle } = useAuth();
    const router = useRouter();
    const [toast, setToast] = useState<{ message: string; type: 'success' | 'error' | 'info'; isVisible: boolean }>({
        message: '',
        type: 'success',
        isVisible: false
    });

    // Handle redirections after login
    useEffect(() => {
        if (!loading && user) {
            if (isAdmin) {
                router.push('/admin');
            }
            // For normal users, we stay on page (or could redirect home)
            // The toast will handle the welcome message
        }
    }, [user, loading, isAdmin, router]);

    const showToast = (message: string, type: 'success' | 'error' | 'info') => {
        setToast({ message, type, isVisible: true });
    };

    const handleGoogleSignIn = async () => {
        try {
            await signInWithGoogle();
            
            // Check if user object is available immediately after await
            const currentUser = await import('@/lib/firebase').then(m => m.auth.currentUser);
            
            if (currentUser) {
                // Heuristic for new user: creation time equals last sign in time
                const isNewUser = currentUser.metadata.creationTime === currentUser.metadata.lastSignInTime;
                const name = currentUser.displayName?.split(' ')[0] || 'User';
                
                if (isNewUser) {
                    showToast(`Welcome, ${name}! Your account has bees created.`, 'success');
                } else {
                    showToast(`Welcome back, ${name}!`, 'success');
                }

                // Redirect home after a delay if not admin
                setTimeout(() => {
                    // Check logic again in case state hasn't updated 
                    // (though isAdmin comes from context which might lag slightly, rely on route)
                   if (!window.location.pathname.includes('/admin')) {
                       router.push('/');
                   }
                }, 2000);
            }

        } catch (error: any) {
            console.error('Sign in error:', error);
            showToast(error.message || 'Failed to sign in. Please try again.', 'error');
        }
    };

    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-600"></div>
            </div>
        );
    }

    // Prepare the view - generic login for everyone
    return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-4">
            <div className="glass-card p-12 rounded-[3rem] text-center max-w-md w-full relative">
                <div className="w-16 h-16 bg-primary-100 dark:bg-primary-900/20 rounded-full flex items-center justify-center mx-auto mb-6">
                    <LogIn className="h-8 w-8 text-primary-600" />
                </div>
                <h1 className="text-3xl font-black mb-4">Welcome</h1>
                <p className="text-slate-500 mb-8">
                    Sign in to access your account, book appointments, or manage your queue.
                </p>
                
                <button
                    onClick={handleGoogleSignIn}
                    className="w-full bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 hover:border-primary-300 text-slate-900 dark:text-white font-bold py-4 px-6 rounded-2xl transition-all flex items-center justify-center gap-3 group"
                >
                    <svg className="w-6 h-6" viewBox="0 0 24 24">
                        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                    </svg>
                    <span>Login / Sign Up with Google</span>
                </button>

                <p className="text-xs text-slate-400 mt-6">
                    By continuing, you agree to our Terms of Service
                </p>
            </div>

            <Toast 
                message={toast.message}
                type={toast.type}
                isVisible={toast.isVisible}
                onClose={() => setToast(prev => ({ ...prev, isVisible: false }))}
            />
        </div>
    );
}
